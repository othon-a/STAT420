...
# there is an issue in the dataset with duplicate yields recording multiple rainfall amounts
# this aggregates duplicates by averaging rainfall into a single record
df = aggregate(
  temp_c ~ country + crop + year + yield_hg_ha + rain_mm + pesticides_t,
  data = df,
  FUN = mean
)
...
# adds UN subregions as a column
subregions = read_csv("subregions_df.csv")
df_with_gdp = merge(
  df_with_gdp,
  subregions[, c("country", "subregion")],
  by = "country",
  all.x = TRUE
)
